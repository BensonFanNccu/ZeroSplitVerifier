...
> **狀態**：✅ **完成**（grid 於 2026-06-20 跑完，80 配置零失敗，約 2.25 天）。Tables 5.3/5.1/5.4 全比對、Figs 5.1–5.4 已生成。5.2/5.5 依範圍未納入。
...
### 4.3 Table 5.4 — LSTM @ MNIST　✅ 完整（16/16）— **重現最差的一張表**
**一致性**：平均 |Δ×|=**24.2**（max 42）——比 RNN-MNIST(2.4)、CIFAR(0.1) **差一個數量級**。
```
每格 ×/↑/%       h4          h8          h16         h32
 m=1  論 44/11/25 40/22/55 27/23/85 16/14/88   我 20/ 5/25  6/ 5/83  1/0/0  0/0/--
 m=2  論 50/19/38 49/38/78 43/32/74 34/23/68   我 17/ 5/29 11/ 4/36  1/0/0  2/0/0
 m=4  論 50/24/48 50/23/46 50/24/48 50/18/36   我 28/11/39 25/ 9/36 15/4/27  9/2/22
 m=7  論 50/11/22 50/15/30 50/ 8/16 50/ 8/16   我 48/ 5/10 45/17/38 44/7/16 43/11/26
```
**分析（重要發現）**：
- **`×`（觸發數）系統性、大幅偏低**，尤其**大 hidden + 短序列**幾乎不觸發（h32/m1：論文 ×16 vs 我們 ×0；h16/m2：論文 ×43 vs 我們 ×1）。
- 這是 **D5 的最強放大**：RNN 的 × 幾乎吻合，LSTM 卻嚴重偏離 → **LSTM 認證對「確切訓練權重」極度敏感**，而 LSTM 訓練（50 epochs + dropout 0.5 + StepLR、**無 seed**、論文未報超參數/accuracy）使我們的權重與論文差異最大。
- 長序列 **m=7 收斂較好**（×43–48 vs 50）：序列夠長時幾乎所有樣本都失敗 baseline，與確切權重較無關。
- **成本**：LSTM 極貴，單配置 1,350–34,080 秒（h16/m7 達 9.5 小時），印證論文「LSTM 比 vanilla RNN 慢 2–3 個數量級」。
- **結論**：Table 5.4 **無法可靠重現逐格數值**（結構性，因 D5），但「ZeroSplit 在 LSTM 上有效（% 非零）」與「LSTM 遠慢於 RNN」的定性結論成立。
...
| Table 5.4（LSTM）| 🟠 **僅定性可重現** | 逐格數值不可靠（|Δ×|=24.2，D5）；極耗時（單格達 9.5h、全表 ~2 天）；「ZeroSplit 有效 + LSTM≫RNN」成立 |
...
*（本報告為最終版。完整 80 配置結果於 `review_workspace/results/`；圖於 `results/figures/`；逐配置進度於 `logs/progress.jsonl`。）*
